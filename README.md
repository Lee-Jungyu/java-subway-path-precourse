...
## 🔨 기능 목록
- java-subway-map-precourse 코드, 기능 활용
- 지하철 구간 관련 기능
  - 이전 구간, 이후 구간의 거리, 소요시간 추가
- 경로 조회 기능
  - 출발역과 도착역이 같다면 ERROR
  - 출발역 혹은 도착역이 등록되지 않았으면 ERROR
  - 최단 거리 경로 조회 기능
    - 다익스트라 알고리즘 사용해서 최단 거리 경로 조회
  - 최소 시간 경로 조회 기능
    - 다익스트라 알고리즘 사용해서 최소 시간 경로 조회
- 초기 설정
  - 지하철 역으로 교대역, 강남역, 역삼역, 남부터미널역, 양재역, 양재시민의숲역, 매봉역을 우선 등록
  - 지하철 노선으로 2호선, 3호선, 신분당선을 우선 등록
  - 각 구간을 다음과 같이 우선 등록
    - 2호선: 교대역 - ( 2km / 3분 ) - 강남역 - ( 2km / 3분 ) - 역삼역
    - 3호선: 교대역 - ( 3km / 2분 ) - 남부터미널역 - ( 6km / 5분 ) - 양재역 - ( 1km / 1분 ) - 매봉역
    - 신분당선: 강남역 - ( 2km / 8분 ) - 양재역 - ( 10km / 3분 ) - 양재시민의숲역
